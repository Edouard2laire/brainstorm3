...
for iResFile = 1:length(ResultsFiles)
    ...
    for k = 1:length(sScouts)
        ...
        if (nComponents == 1)
            ...
            isNIRS = ~isempty(strfind(TestTags, 'NIRS')) || ~isempty(strfind(TestTags, 'Summed_sensitivities')) || ...
                     ( isfield( GlobalData.DataSet(iDS).Results(iResult), 'DisplayUnits') && any(cellfun(@(p) ~isempty(strfind(GlobalData.DataSet(iDS).Results(iResult).DisplayUnits, p)), {'mol.l-1', 'OD'})) ) || ...
                     (~isempty(GlobalData.DataSet(iDS).Channel) && ~isempty(GlobalData.DataSet(iDS).Results(iResult).GoodChannel) && ismember('NIRS', {GlobalData.DataSet(iDS).Channel(GlobalData.DataSet(iDS).Results(iResult).GoodChannel).Type}));
