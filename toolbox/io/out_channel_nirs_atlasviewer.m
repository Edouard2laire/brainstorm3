function out_channel_nirs_atlasviewer(BstFile, OutputFile, MriFile)
% out_channel_nirs_atlasviewer: Export a Brainstorm channel file in 
% AtlasViewer files format.
%
% USAGE:  out_channel_nirs_atlasviewer( BstFile,    OutputFile, MriFile );
%         out_channel_nirs_atlasviewer( ChannelMat, OutputFile, MriFile );
%
% INPUT: 
...
if isfield(ChannelMat.Nirs, 'Wavelengths')
    nirs.SD.Lambda = ChannelMat.Nirs.Wavelengths;
else
    nirs.SD.Lambda = ChannelMat.Nirs.Hb;
end
    ...
    if nargin >= 3
        ...
        if ~( isempty(sMri) || isempty(sMri.InitTransf))
            src_coords = cs_convert(sMri, 'scs', 'mni', src_coords)*1000;
            det_coords = cs_convert(sMri, 'scs', 'mni', det_coords)*1000;
                if ~isempty(fidu_coords)
                    fidu_coords = cs_convert(sMri, 'scs', 'mni', fidu_coords)*1000;
                end

        else
            src_coords = src_coords * 1000;
            det_coords = det_coords * 1000;
            fidu_coords = fidu_coords*1000;
        end
    end 
    ...
    for imeasure=1:length(nirs.SD.Lambda)
        for ipair=1:size(pair_sd_indexes, 1)
            ml(ichan, 1) = pair_sd_indexes(ipair, 1);
            ml(ichan, 2) = pair_sd_indexes(ipair, 2);
            ml(ichan, 3) = 1;
            ml(ichan, 4) = imeasure;
            ichan = ichan + 1;
        end
    ...
    nirs.SD.AnchorList = fidu_coords;
    save(OutputFile, '-struct', 'nirs');
